...
class _UserStatisticsScreenState extends State<UserStatisticsScreen> {
  ...
  void initState() {
    super.initState();
    context.read<UserStatisticsBloc>().add(LoadUserStatistics());
  }
        ...
        child: BlocBuilder<UserStatisticsBloc, UserStatisticsState>(
          builder: (context, state) {
            ...
            if (stats == null || stats.weeklyCaloriesConsumption.isEmpty) {
              return Center(child: Text(AppLocalizations.of(context)!.statistsMissing));
            }
          ...
          bottomTitles: AxisTitles(
            sideTitles: SideTitles(
              ...
              getTitlesWidget: (value, _) {
                ...
                final labelIndex = day.weekday - 1;

                return Padding(
                  padding: const EdgeInsets.only(top: 4),
                  child: Text(labels[labelIndex]),
                );
              },
            ),
          ),
  ...
  Widget _buildGoalComparisonChart(UserStatistics stats) {
    ...
    if (actualMin <= 300) {
      ...
      minY = (actualMin - 200).toDouble().clamp(0.0, double.infinity);
    }
      ...
      LineChartData(
        ...
        lineBarsData: [
          LineChartBarData(
            spots: List.generate(
              days.length,
              (i) => FlSpot(i.toDouble(), stats.targetCalories.toDouble()),
            ),
            isCurved: false,
            color: Colors.green,
            barWidth: 2,
